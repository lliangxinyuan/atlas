...
APP_ERROR InitAscend(int32_t deviceID, aclrtContext &context, aclrtStream &stream)
{
    ...
    Statistic::SetStatisticEnable(true);
...
APP_ERROR DataInteraction()
{
    ...
    char *devPtr = nullptr;
    APP_ERROR ret = aclrtMalloc((void **)&devPtr, str.length(), ACL_MEM_MALLOC_NORMAL_ONLY);
    if (ret != APP_ERR_OK) {
    ...
    ret = aclrtMallocHost((void **)&hostPtr, str.length());
    if (ret != APP_ERR_OK) {
    ...
    ret = aclrtMemcpy(devBuf.get(), str.length(), str.c_str(), str.length(), ACL_MEMCPY_HOST_TO_DEVICE);
    if (ret != APP_ERR_OK) {
    ...
    ret = aclrtMemcpy(hostBuf.get(), str.length(), devBuf.get(), str.length(), ACL_MEMCPY_DEVICE_TO_HOST);
    if (ret != APP_ERR_OK) {
...
int main(int argc, char **argv)
{
    ...
    Statistic process;
    process.RunTimeStatisticStart("TotalRuntime");
    ...
    process.RunTimeStatisticStop();
    LogInfo << "TotalRuntime: " << process.GetRunTimeAvg(false) << "ms";
