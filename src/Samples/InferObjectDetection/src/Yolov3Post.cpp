                 ...
                 int netHeight)
{
    ...
    for (int i = 0; i < featLayerNum; ++i) {
        ...
        for (int j = startIdx; j < endIdx; ++j) {
            outputLayer.anchors[idx++] = g_biases[j];
        }
...
/*
 * @description: Select the highest confidence class label for each predicted box and save into detBoxes with NCHW format
 ...
 */
void SelectClassNCHW(std::shared_ptr<void> netout, NetInfo info, std::vector<DetectBox>& detBoxes, int stride, OutputLayer layer)
{
...
/*
 * @description: Select the highest confidence class label for each predicted box and save into detBoxes with NHWC format
 * @param netout  The feature data which contains box coordinates, objectness value and confidence of each class
 * @param info  Yolo layer info which contains class number, box dim and so on
 * @param detBoxes  DetectBox vector where all DetectBoxes's confidences are greater than threshold
 * @param stride  Stride of output feature data
 * @param layer  Yolo output layer
 */
void SelectClassNHWC(std::shared_ptr<void> netout, NetInfo info, std::vector<DetectBox>& detBoxes, int stride, OutputLayer layer)
{
    const int offsetY = 1;
    const int offsetWidth = 2;
    const int offsetHeight = 3;
